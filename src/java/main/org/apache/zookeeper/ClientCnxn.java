...
public class ClientCnxn {
     ...
     * 并非Packet中所有的属性都在客户端与服务端之间进行网络传输，只会将requestHeader、request、readOnly
     * 三个属性序列化，并生成可用于底层网络传输的ByteBuffer，其他属性都保存在客户端的上下文中，不会进行与
     * 服务端之间的网络传输。
     */
    static class Packet {
    ...
    private void finishPacket(Packet p) {
        ...
        if (p.cb == null) {
            ...
            synchronized (p) {
                p.finished = true;
                // 唤醒org.apache.zookeeper.ClientCnxn.submitRequest
                // packet.wait();
                p.notifyAll();
            }
        } else {
    ...
    class SendThread extends ZooKeeperThread {
        ...
        void primeConnection() throws IOException {
            ...
            synchronized (outgoingQueue) {
                ...
                // 将连接请求对象conReq加入到outgoingQueue队列中
        ...
        public void run() {
            ...
            // 重试逻辑就是通过while循环，当超时时，抛出异常，利用try-catch逻辑获取
            // 该异常，并进行下一次的while循环逻辑
            while (state.isAlive()) {
                try {
                    if (!clientCnxnSocket.isConnected()) {
                        /**
                         * 这里判断是否是第一次连接，如果第一次连接超时，通过重试机制
                         * 会进入该逻辑，先进行随机时间的睡眠，然后依然会执行
                         * hostProvider.next()方法获取连接地址（如果有多个）
                         */
                        if(!isFirstConnect){
                            try {
                        ...
                        if (rwServerAddress != null) {
                            ...
                            // hostProvider.next()方法获取连接地址（如果有多个，会获取下一个连接
                            // 进行连接，内部利用取模运算）
                    ...
                    // 如果连接失败，看是否超过了connectTimeout，超过了则抛异常
                    // SessionTimeoutException，但这个异常会被上层catch住，尝试重连
                    if (to <= 0) {
                        String warnInfo;
        ...
        void onConnected(int _negotiatedSessionTimeout, long _sessionId,
            ...
            // 客户端连接成功以后，会触发EventType.None的事件
            eventThread.queueEvent(new WatchedEvent(
                    Watcher.Event.EventType.None,
                    eventState, null));
        }
    ...
    public ReplyHeader submitRequest(RequestHeader h, Record request,
        ...
        // 将request请求包装成packet，放入outgoingQueue队列中
