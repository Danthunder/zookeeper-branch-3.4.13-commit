...
public class SyncRequestProcessor extends ZooKeeperCriticalThread implements RequestProcessor {
    ...
    @Override
    public void run() {
        // 持久化，每1000个请求，将toFlush队列中的请求flush到dataLogDir中的事务日志文件中
        try {
            int logCount = 0;
            ...
            while (true) {
                ...
                if (si != null) {
                    ...
                    if (zks.getZKDatabase().append(si)) {
                        logCount++;
                        // snapCount快照数量可在配置文件中配置
                        if (logCount > (snapCount / 2 + randRoll)) {
                            setRandRoll(r.nextInt(snapCount/2));
                            // roll the log
                            // rollLog中也会将logStream进行flush后并置空
    ...
    public void processRequest(Request request) {
        ...
        // 将当前请求加入到LinkedBlockingQueue<Request> queuedRequests队列中
