...
public class FileTxnSnapLog {
    ...
    public long restore(DataTree dt, Map<Long, Integer> sessions, 
        ...
        snapLog.deserialize(dt, sessions); // 从快照中反序列化数据到内存DataTree
     ...
     * @throws IOException
     *
     * fastForwardFromEdits:用于重放快照之后的事务。
     * 假设事务1,2,3,4时，进行了打快照的操作，此后继续进行事务5,6,7,8。此时机器宕机。
     * 当该机器重启恢复时，会执行snapLog.deserialize(dt, sessions)方法，从快照中
     * 恢复数据到DataTree，即1~4的事务已经恢复，但5~8的事务并没有恢复。
     * 在该方法中，首先记录最近一次快照恢复的最后一个事务ID，这里是4，然后在事务日志中
     * 5开始，依次重放（playback）剩余所有的事务，更新到DataTree中，从而完成剩余事务
     * 的恢复。
     */
