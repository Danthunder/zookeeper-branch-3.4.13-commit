...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public synchronized void start() {
        // 从快照文件中载入数据到内存，并重放事务日志的剩余事务到内存DataTree，
        // 防止因机器宕机而未来得及进行打快照造成的事务丢失
        // 注意打快照是在SyncRequestProcessor（持久化事务/打快照）中进行
        // 并且是累计多个事务后打一次快照，
        loadDataBase();
        // 开启读取数据线程，接收客户端的请求
        cnxnFactory.start();
        // 执行领导者选举，确定服务器的角色，再针对不同的服务器角色进行初始化
        startLeaderElection();
        // 本类的run方法
        super.start();
    }
    ...
    synchronized public void startLeaderElection() {
    	try {
    	    // 生成投票，投给自己
            // currentVote属性可以理解为每台服务器的投票箱，不同的
            // zookeeper服务器的投票箱应该选票数量一致
            ...
            while (running) {
                ...
                case FOLLOWING:
                    ...
                    try {
                        ...
                        follower.followLeader(); // follower向leader同步数据
                    } catch (Exception e) {
                        LOG.warn("Unexpected exception",e);
                    } finally {
                        follower.shutdown();
                        setFollower(null);
                        // 只要服务器在运行过程中出现了异常，就会设置成LOOKING状态
                        setPeerState(ServerState.LOOKING);
                    }
                    break;
                case LEADING:
