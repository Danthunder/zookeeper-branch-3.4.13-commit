...
public class Learner {       
    ...
    protected long registerWithLeader(int pktType) throws IOException{
    	...
    	// 构造请求QuorumPacket--Leader.FOLLOWERINFO
        ...
        /*
         * Add sid to payload
         * 将learner自己的Sid放入LearnerInfo中，发从给leader
         * leader会根据该learner的Sid信息更新自己的epoch
         */
        ...
        // 发送请求QuorumPacket（LearnerInfo），包括当前服务器上最新的Epoch
        // Epoch可以理解为一届，每次领导者选举完，Epoch就会更新，
        // 当前选举出的领导者要统一这一届选举的序号
        writePacket(qp, true);

        // 接收读取Leader（learnerHandler）发送的LeaderInfo信息
        readPacket(qp);        
		...
		if (qp.getType() == Leader.LEADERINFO) {
        	...
        	// 解析完LEADERINFO，构造ACKEPOCH的QP
        	QuorumPacket ackNewEpoch = new QuorumPacket(Leader.ACKEPOCH, lastLoggedZxid, epochBytes, null);
        	// 向Leader（learnerHandler）发送ACK
    ...
    protected void syncWithLeader(long newLeaderZxid) throws IOException, InterruptedException{
        ...
        synchronized (zk) {
            if (qp.getType() == Leader.DIFF) {
                ...
                // 执行truncateLog方法，删除Learner多余的数据，和Leader的lastZxid保持一致
            ...
            while (self.isRunning()) {
                // 在while循环中不断接收Leader发送的数据（commit指令和其携带的数据）
                // 并将数据存入packetsCommitted队列中
                readPacket(qp);
                switch(qp.getType()) {
                ...
                case Leader.COMMIT:
                    ...
                    if (!writeToTxnLog) {
                        ...
                        // 需要事务日志，将该事务ID加入到packetsCommitted:LinkedList<Long>中
                        // 在退出循环后（接收到服务端发来的Leader.UPTODATE指令后），遍历该队列依
                        // 次执行commit
        ...
        // 同步完数据后，启动Learner（启动zookeeper），接收客户端发来的请求
        zk.startup();
        /*
