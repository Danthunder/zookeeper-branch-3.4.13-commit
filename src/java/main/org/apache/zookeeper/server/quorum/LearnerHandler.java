...
public class LearnerHandler extends ZooKeeperThread {
    ...
    public void run() {
        try {
            ...
            // 读取learner向leader发送的QuorumPacket请求
            ...
            if (learnerInfoData != null) {
                //接收learner发送过来的LearnerInfo,获取其sid(serverId)
            ...
            /**
             * Zxid说明：在ZAB协议的事务编号设计中，Zxid是一个64位的数字，其中
             * 低32位是一个简单的单调递增的计数器，针对客户端每一个事务请求，计数器+1
             * 高32位则代表Leader周期epoch的编号，每当选举产生一个新的Leader服务器，
             * 就会从这个Leader服务器上取出其本地日志中最大事务的Zxid，并从中取出epoch
             * 值，然后+1，以此作为新的epoch，并将低32位从0开始计数
             *
             * 这里记录当前learner的最新的epoch
             * 先选出leader，在确定届号
             */
            ...
            // this.getSid()获取的是learner的Sid，而非leader的Sid
            // Sid就是配置文件myid中的取值
            // getEpochToPropose逻辑就是：判断如果learner的epoch比自己leader高，更新自己的
            // epoch为newEpoch
            ...
            if (this.getVersion() < 0x10000) {
                ...
                // 向learner发送leader状态,以 LEADERINFO 的形式
                // zxid根据新的newEpoch创建得到
            ...
            // peerLastZxid是follower的最后一次事务ID
            ...
            try {
                ...
                // 从committedLog中，获取历史提交记录
                LinkedList<Proposal> proposals = leader.zk.getZKDatabase().getCommittedLog();

                if (peerLastZxid == leader.zk.getZKDatabase().getDataTreeLastProcessedZxid()) {
                    // Follower is already sync with us, send empty diff
                    // 如果learner的最新事务ID(zxid)和leader当前最新的是相等的，则没有diff
                    ...
                    if ((maxCommittedLog >= peerLastZxid)
                        ...
                        // 说明当前follower的事务ID介于minCommittedLog和maxCommittedLog之间
                        // 需要向其同步peerLastZxid+1~maxCommittedLog之间的事务
                        ...
                        for (Proposal propose: proposals) {
                            ...
                            if (propose.packet.getZxid() <= peerLastZxid) {
                                ...
                                /**
                                 * 将请求加入到queuedPackets:LinkedBlockingQueue<QuorumPacket>中
                                 * The packets to be sent to the learner.
                                 * 这里注意，Leader.TRUNC和Leader.DIFF类型的请求会加入到该队列中，
                                 * 在后面会创建一个线程，执行sendPackets()方法，从该队列中获取请求
                                 * 并最终发送
                                 * 但Leader.SNAP类型则直接通过socket发送，不加入到该队列中
                                 *
                                 */
                        ...
                        // learner的事务ID(zxid)超过Leader的事务部分，进行回滚
            ...
            if (packetToSend == Leader.SNAP) {
                //如果是SNAP同步，获取最大的zxid事务ID
                zxidToSend = leader.zk.getZKDatabase().getDataTreeLastProcessedZxid();
            }
            // 先发送类型，再发送数据。oa负责发送数据
            ...
            while (true) {
                // 正常交互，处理learner的请求等
                // 不停的从ia中获取从Learner发过来的QuorumPacket
                qp = new QuorumPacket();
                ia.readRecord(qp, "packet");
                ...
                case Leader.REVALIDATE:
                    ...
                    break;
                case Leader.REQUEST:
                    // 当客户端连接到Follower，并发送写请求时，该Follower会向
                    // Leader发送REQUEST指令，将客户端的写请求转发给Leader
                    // 这里有一个问题，作为Leaner它是怎么把这个request发出来的，
                    // 应该要用到QuorumServer中的同步端口
