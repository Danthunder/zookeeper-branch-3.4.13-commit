...
public class QuorumPeerConfig {
    ...
    public void parse(String path) throws ConfigException {
        ...
        try {
            ...
            try {
                cfg.load(in); // 加载配置文件，构造Properties对象
            } finally {
                in.close();
            }
            // 解析配置文件
    ...
    throws IOException, ConfigException {
        ...
        if (servers.size() == 0) {
            ...
            if(serverGroup.size() > 0){
                 ...
                 */
                /**
                 * 重要方法QuorumMaj：过半机制验证，QuorumMaj实现QuorumVerifier接口
                 * 构造方法中传入的是servers的数量（排除observers），构造方法内部初始化
                 * half = servers.size() / 2; 用于后续containsQuorum方法判断集群是否
                 * 是合法集群
                 */
