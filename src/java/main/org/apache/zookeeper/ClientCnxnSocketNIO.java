...
public class ClientCnxnSocketNIO extends ClientCnxnSocket {
    ...
    void doIO(List<Packet> pendingQueue, LinkedList<Packet> outgoingQueue, ClientCnxn cnxn)
      throws InterruptedException, IOException {
        ...
        if (sockKey.isReadable()) { // 读就绪
            ...
            if (rc < 0) { // 如果 rc < 0,表示读到末尾了,这种情况出现在连接关闭的时候
            ...
            if (!incomingBuffer.hasRemaining()) {
                ...
                if (incomingBuffer == lenBuffer) {
                    ...
                    readConnectResult(); //读取连接请求的结果，即 connect 回复
                    enableRead();
        ...
        if (sockKey.isWritable()) { // 写就绪
            synchronized(outgoingQueue) {
                ...
                if (p != null) {
                    ...
                    sock.write(p.bb); // 数据发送到服务端
                    if (!p.bb.hasRemaining()) { // 缓冲区数据已经发送完毕
                        sentCount++;
                        outgoingQueue.removeFirstOccurrence(p); // outgoingQueue移除首元素
                        if (p.requestHeader != null
                                && p.requestHeader.getType() != OpCode.ping
                                && p.requestHeader.getType() != OpCode.auth) {
                            synchronized (pendingQueue) {
                                // 加入待回复的队列
                                // pendingQueue保存需要等待服务端返回结果的package请求
                                // 获取服务端处理结果是用到pendingQueue队列
                                pendingQueue.add(p);
                            }
                        }
                    }
    ...
    throws IOException {
        ...
        // 如果没有立即connect上，那么就在org.apache.zookeeper.ClientCnxnSocketNIO.doTransport
        // 中等待SocketChannel finishConnect再调用
